...
def orientation(p, q, r):
    ...
    val = (np.float64(q.y - p.y) * (r.x - q.x)) - \
          (np.float64(q.x - p.x) * (r.y - q.y))
    if val > 0:
        return 1
    elif val < 0:
        return 2
...
def build_rect(pt1, pt2, width):
    """Build a rectangle around a line segment."""
    line_angle = np.arctan2(pt2.y - pt1.y, pt2.x - pt1.x)
    angle1 = line_angle + np.pi / 2
    angle2 = line_angle - np.pi / 2
    rect_pt1 = Point2D(
        pt1.x + width / 2.0 * np.cos(angle1),
        pt1.y + width / 2.0 * np.sin(angle1))
    rect_pt2 = Point2D(
        pt1.x + width / 2.0 * np.cos(angle2),
        pt1.y + width / 2.0 * np.sin(angle2))
    rect_pt3 = Point2D(
        pt2.x + width / 2.0 * np.cos(angle1),
        pt2.y + width / 2.0 * np.sin(angle1))
    rect_pt4 = Point2D(
        pt2.x + width / 2.0 * np.cos(angle2),
        pt2.y + width / 2.0 * np.sin(angle2))
...
def compute_cost(chain1, chain2, w_e, w_c, mode):
    """Compute cost of connecting two chains."""
    ...
    if (total_cost is np.nan
            or cost_curvature_1 is np.nan
            or cost_curvature_2 is np.nan):
        print('total cost is nan!')
        print('chain1 =', chain1)
        print('chain2 =', chain2)
        curv_cost = (
            np.abs(cost_curvature_1)
            + np.abs(cost_curvature_2)) / 2.0
        print(
            'euclidean cost = {}, w_e*cost = {}; '
            'curvature cost = {}, w_c*cost = {}'.format(
                cost_euclidean, w_e * cost_euclidean,
                curv_cost, w_c * curv_cost))
    return total_cost


# partial implementation of paper
# "Deformable One-Dimensional Object Detection for
# Routing and Manipulation"
# paper link:
# https://ieeexplore.ieee.org/abstract/document/9697357
def extract_connected_skeleton(
        visualize_process, mask,
        img_scale=10, seg_length=3, max_curvature=30):
    """Extract connected skeleton chains from a mask image."""
    # smooth image (use smaller filter to preserve thin
    # DLO features)
    im = Image.fromarray(mask)
    smoothed_im = im.filter(ImageFilter.ModeFilter(size=5))
    mask = np.array(smoothed_im)

    # resize if necessary for better skeletonization
    mask = cv2.resize(mask, (
        int(mask.shape[1] / img_scale),
        int(mask.shape[0] / img_scale)))
    ...
    contours, _ = cv2.findContours(
        gray, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)[-2:]

    chains = []

    for _a, contour in enumerate(contours):
        ...
        for i, _coord in enumerate(contour):
            if i == len(contour) - 1:
                if len(chain) != 0:
                    chains.append(chain)
                break
            ...
            mask = cv2.line(
                mask,
                tuple(contour[i][0]),
                tuple(contour[i + 1][0]), 255, 1)

            if cur_seg_start_point is None:
                cur_seg_start_point = contour[i][0].copy()

            dx = contour[i][0][0] - cur_seg_start_point[0]
            dy = contour[i][0][1] - cur_seg_start_point[1]
            if np.sqrt(dx**2 + dy**2) <= seg_length:
                continue
            ...
            cur_segment_dir = [
                cur_seg_end_point[0]
                - cur_seg_start_point[0],
                cur_seg_end_point[1]
                - cur_seg_start_point[1]]
            if last_segment_dir is None:
                last_segment_dir = cur_segment_dir.copy()

            elif (np.dot(
                    np.array(cur_segment_dir),
                    np.array(last_segment_dir))
                  / (np.sqrt(
                      cur_segment_dir[0]**2
                      + cur_segment_dir[1]**2)
                     * np.sqrt(
                         last_segment_dir[0]**2
                         + last_segment_dir[1]**2))
                  >= np.cos(
                      max_curvature / 180 * np.pi)):
                if len(chain) == 0:
                    chain.append(
                        cur_seg_start_point.tolist())
                    chain.append(
                        cur_seg_end_point.tolist())
                else:
                    chain.append(
                        cur_seg_end_point.tolist())

                cur_seg_start_point = \
                    cur_seg_end_point.copy()
                last_segment_dir = \
                    cur_segment_dir.copy()
    ...
    print('Finished contour traversal. '
          'Pruning extracted chains...')

    if visualize_process:
        mask = np.zeros(
            (gray.shape[0], gray.shape[1], 3), np.uint8)
        for chain in chains:
            color = (
                int(np.random.random() * 200) + 55,
                int(np.random.random() * 200) + 55,
                int(np.random.random() * 200) + 55)
            for i in range(0, len(chain) - 1):
                mask = cv2.line(
                    mask, chain[i], chain[i + 1],
                    color, 1)
    ...
    rect_width = 3
    for chain in chains:
        all_chain_length.append(np.sum(np.sqrt(np.sum(
            np.square(np.diff(np.array(chain), axis=0)),
            axis=1))))
        for i in range(0, len(chain) - 1):
            line_seg_to_rect_dict[
                (tuple(chain[i]), tuple(chain[i + 1]))
            ] = build_rect(
                Point2D(chain[i][0], chain[i][1]),
                Point2D(chain[i + 1][0], chain[i + 1][1]),
                rect_width)
    ...
    for _i in range(0, len(chains)):
        ...
        for j in range(0, len(sorted_chains) - 1):
            ...
            new_test_chain = []
            for ll in range(0, len(test_chain) - 1):
                rect_test_seg = line_seg_to_rect_dict[
                    (tuple(test_chain[ll]),
                     tuple(test_chain[ll + 1]))]
                no_overlap = True
                for k in range(0, len(cur_chain) - 1):
                    rect_cur_seg = line_seg_to_rect_dict[
                        (tuple(cur_chain[k]),
                         tuple(cur_chain[k + 1]))]
                    if check_rect_overlap(
                            rect_cur_seg, rect_test_seg):
                        no_overlap = False
                        break
                if no_overlap:
                    if len(new_test_chain) == 0:
                        new_test_chain.append(
                            test_chain[ll])
                        new_test_chain.append(
                            test_chain[ll + 1])
                    else:
        ...
        for chain in leftover_chains:
            if len(chain) != 0:
                all_chain_length.append(np.sum(np.sqrt(
                    np.sum(np.square(np.diff(
                        np.array(chain), axis=0)),
                        axis=1))))
            else:
    ...
    for i in range(0, len(pruned_chains)):
        for j in range(0, len(pruned_chains)):
            if i == j:
                ...
                cost_matrix[2 * i, 2 * j] = compute_cost(
                    pruned_chains[i], pruned_chains[j],
                    w_e, w_c, 0)
                cost_matrix[2 * i, 2 * j + 1] = \
                    compute_cost(
                        pruned_chains[i], pruned_chains[j],
                        w_e, w_c, 1)
                cost_matrix[2 * i + 1, 2 * j] = \
                    compute_cost(
                        pruned_chains[i], pruned_chains[j],
                        w_e, w_c, 2)
                cost_matrix[2 * i + 1, 2 * j + 1] = \
                    compute_cost(
                        pruned_chains[i], pruned_chains[j],
                        w_e, w_c, 3)
    ...
    cost_matrix[
        ...
        matrix_size - 2:matrix_size] = 100000

    _row_idx, col_idx = linear_sum_assignment(cost_matrix)
    cur_idx = col_idx[_row_idx[-1]]
    ordered_chains = []
    ...
    while True:
        ...
        if (next_idx == matrix_size - 1
                or next_idx == matrix_size - 2):
            break
        cur_idx = next_idx
    ...
    if visualize_process:
        ...
        for i in range(0, len(ordered_chains)):
            ...
            mask = cv2.line(
                mask, pt1, pt2, (255, 255, 255), 2)
            mask = cv2.circle(
                mask, pt1, 3, (255, 255, 255))
            mask = cv2.circle(
                mask, pt2, 3, (255, 255, 255))
...
def rotation_matrix_from_vectors(vec1, vec2):
    ...
    :return mat: A transform matrix (3x3) which when
        applied to vec1, aligns it with vec2.
    """
    a, b = (
...
def ndarray2MarkerArray(Y, marker_frame,
                        node_color, line_color):
    """Convert ndarray to MarkerArray for visualization."""
    results = MarkerArray()
    for i in range(0, len(Y)):
        ...
        mid = (Y[i] + Y[i + 1]) / 2
        cur_line_result.pose.position.x = float(mid[0])
        cur_line_result.pose.position.y = float(mid[1])
        cur_line_result.pose.position.z = float(mid[2])
