  ...
  int dlo_pixel_width)
{
  ...
  std::sort(
    indices_vec.begin(), indices_vec.end(),
  ...
  for (int idx : indices_vec) {
    ...
    if (pt1_in_bounds && projected_edges.at<uchar>(row_1, col_1) == 0) {
      if (shortest_node_pt_dists[idx] <= visibility_threshold) {
        if (std::find(
            result.visible_nodes.begin(), result.visible_nodes.end(),
            idx) == result.visible_nodes.end())
        {
      ...
      if (std::find(
          ...
          idx) == result.not_self_occluded_nodes.end())
      {
    ...
    if (pt2_in_bounds && projected_edges.at<uchar>(row_2, col_2) == 0) {
      if (shortest_node_pt_dists[idx + 1] <= visibility_threshold) {
        if (std::find(
            result.visible_nodes.begin(), result.visible_nodes.end(),
            idx + 1) == result.visible_nodes.end())
        {
      ...
      if (std::find(
          result.not_self_occluded_nodes.begin(), result.not_self_occluded_nodes.end(),
          idx + 1) == result.not_self_occluded_nodes.end())
      {
    ...
    cv::line(
      projected_edges, cv::Point(col_1, row_1), cv::Point(col_2, row_2), cv::Scalar(
        255), dlo_pixel_width);
  }
