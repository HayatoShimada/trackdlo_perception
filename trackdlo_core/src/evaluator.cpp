...
evaluator::evaluator() {}
evaluator::evaluator(
  int length, int trial, int pct_occlusion, std::string alg, int bag_file,
...
MatrixXd evaluator::get_ground_truth_nodes(Mat rgb_img, pcl::PointCloud<pcl::PointXYZRGB> cloud_xyz)
{
  ...
  cv::inRange(
    cur_image_hsv, cv::Scalar(lower_blue[0], lower_blue[1], lower_blue[2]),
    cv::Scalar(upper_blue[0], upper_blue[1], upper_blue[2]), mask_blue);
  cv::inRange(
    cur_image_hsv, cv::Scalar(lower_red_1[0], lower_red_1[1], lower_red_1[2]),
    cv::Scalar(upper_red_1[0], upper_red_1[1], upper_red_1[2]), mask_red_1);
  cv::inRange(
    cur_image_hsv, cv::Scalar(lower_red_2[0], lower_red_2[1], lower_red_2[2]),
    cv::Scalar(upper_red_2[0], upper_red_2[1], upper_red_2[2]), mask_red_2);
  cv::inRange(
    cur_image_hsv, cv::Scalar(
      lower_yellow[0], lower_yellow[1],
      lower_yellow[2]),
    cv::Scalar(upper_yellow[0], upper_yellow[1], upper_yellow[2]), mask_yellow);
  ...
  for (cv::KeyPoint key_point : keypoints_markers) {
    auto keypoint_pc =
      cloud_xyz(static_cast<int>(key_point.pt.x), static_cast<int>(key_point.pt.y));
...
double evaluator::calc_min_distance(
  MatrixXd A, MatrixXd B, MatrixXd E,
  MatrixXd & closest_pt_on_AB_to_E)
{
  MatrixXd AB = B - A;
  MatrixXd AE = E - A;
...
double evaluator::get_piecewise_error(MatrixXd Y_track, MatrixXd Y_true)
{
  ...
  for (int idx = 0; idx < Y_track.rows(); idx++) {
    ...
    for (int i = 0; i < Y_true.rows() - 1; i++) {
      ...
      double dist_i = calc_min_distance(
        Y_true.row(i), Y_true.row(i + 1), Y_track.row(
          idx), closest_pt_i);
...
double evaluator::compute_and_save_error(MatrixXd Y_track, MatrixXd Y_true)
{
  ...
  std::string dir;
  if (bag_file_ == 0) {
    dir = save_location_ + alg_ + "_" + std::to_string(trial_) + "_" +
      std::to_string(pct_occlusion_) + "_stationary_error.txt";
  } else if (bag_file_ == 1) {
    dir = save_location_ + alg_ + "_" + std::to_string(trial_) + "_" +
      std::to_string(pct_occlusion_) + "_perpendicular_motion_error.txt";
  } else if (bag_file_ == 2) {
    dir = save_location_ + alg_ + "_" + std::to_string(trial_) + "_" +
      std::to_string(pct_occlusion_) + "_parallel_motion_error.txt";
  } else if (bag_file_ == 4) {
    dir = save_location_ + alg_ + "_" + std::to_string(trial_) + "_" +
      std::to_string(pct_occlusion_) + "_short_rope_folding_error.txt";
  } else if (bag_file_ == 5) {
    dir = save_location_ + alg_ + "_" + std::to_string(trial_) + "_" +
      std::to_string(pct_occlusion_) + "_short_rope_stationary_error.txt";
  } else {
  ...
  if (cleared_file_ == false) {
    ...
    error_list << std::to_string(time_diff - start_record_at_) + " " + std::to_string(
      cur_frame_error) + "\n";
    error_list.close();
    cleared_file_ = true;
  } else {
