...
bool ImagePreprocessor::process(
  const cv::Mat & cur_image_orig, cv::Mat & out_mask,
  cv::Mat & out_cur_image)
{
  ...
  if (use_external_mask_) {
    ...
    if (external_mask_.rows != cur_image_orig.rows || external_mask_.cols != cur_image_orig.cols) {
      cv::resize(
        external_mask_, mask_without_occlusion_block,
        cur_image_orig.size(), 0, 0, cv::INTER_NEAREST);
    } else {
    ...
    if (!multi_color_dlo_) {
      cv::inRange(
        cur_image_hsv, cv::Scalar(lower_[0], lower_[1], lower_[2]),
