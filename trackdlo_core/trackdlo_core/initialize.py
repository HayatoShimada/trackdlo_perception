...
import std_msgs.msg
from trackdlo_core.utils import extract_connected_skeleton
from trackdlo_core.utils import ndarray2MarkerArray
...
class InitTrackerNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter(
            'result_frame_id', 'camera_color_optical_frame')
        self.declare_parameter(
        ...
        self.num_of_nodes = \
            self.get_parameter('num_of_nodes').value
        self.multi_color_dlo = \
            self.get_parameter('multi_color_dlo').value
        camera_info_topic = \
            self.get_parameter('camera_info_topic').value
        rgb_topic = self.get_parameter('rgb_topic').value
        depth_topic = self.get_parameter('depth_topic').value
        self.result_frame_id = \
            self.get_parameter('result_frame_id').value
        self.visualize_initialization_process = \
            self.get_parameter(
                'visualize_initialization_process').value

        hsv_threshold_upper_limit = \
            self.get_parameter('hsv_threshold_upper_limit').value
        hsv_threshold_lower_limit = \
            self.get_parameter('hsv_threshold_lower_limit').value
        ...
        self.use_external_mask = \
            self.get_parameter('use_external_mask').value
        ...
        if self.use_external_mask:
            self.external_mask_sub = self.create_subscription(
                ...
                self.external_mask_callback, 10)
            self.get_logger().info(
                'External mask mode enabled. '
        ...
        self.pc_pub = self.create_publisher(
            PointCloud2, '/trackdlo/init_nodes', 10)
        self.results_pub = self.create_publisher(
            MarkerArray, '/trackdlo/init_nodes_markers', 10)
    ...
    def camera_info_callback(self, info):
        ...
        self.get_logger().info(
            'Received camera projection matrix:')
    ...
    def color_thresholding(self, hsv_image, cur_depth):
        mask_dlo = cv2.inRange(
            hsv_image.copy(),
            self.lower, self.upper).astype('uint8')
        ...
        mask_green = cv2.inRange(
            hsv_image.copy(),
            lower_green, upper_green).astype('uint8')

        # combine masks
        mask = cv2.bitwise_or(
            mask_green.copy(), mask_dlo.copy())
    ...
    def external_mask_callback(self, msg):
        self.external_mask = self.bridge.imgmsg_to_cv2(
            msg, 'mono8')
    ...
    def remove_duplicate_rows(self, array):
        ...
        self.set_parameters([
            Parameter(
                'num_of_nodes',
                Parameter.Type.INTEGER, len(data))])
        return data
    ...
    def callback(self, rgb, depth):
        ...
        hsv_image = cv2.cvtColor(
            cur_image.copy(), cv2.COLOR_RGB2HSV)

        # Process depth image
        cur_depth = self.bridge.imgmsg_to_cv2(
        ...
        if self.use_external_mask:
            ...
            mask = cv2.inRange(
                hsv_image, self.lower, self.upper)
            mask_tip = None
        else:
            mask, mask_tip = self.color_thresholding(
                hsv_image, cur_depth)

        try:
            start_time = time.time()
            mask = cv2.cvtColor(
                mask.copy(), cv2.COLOR_GRAY2BGR)
            ...
            if len(all_pixel_coords) < 10:
                self.get_logger().warn(
                    ...
                    f'({len(all_pixel_coords)}), retrying...')
                return
            elapsed = time.time() - start_time
            self.get_logger().info(
                'Finished extracting chains '
                f'({len(all_pixel_coords)} pts). '
                f'Time: {elapsed:.2f}s')

            all_pixel_coords = (
                np.array(all_pixel_coords) * img_scale)
            all_pixel_coords = np.flip(
                all_pixel_coords, 1)

            pc_z = cur_depth[
                tuple(map(tuple, all_pixel_coords.T))
            ] / 1000.0
            ...
            if (self.multi_color_dlo
                    and mask_tip is not None):
                pixel_value1 = mask_tip[
                    pixel_y[-1], pixel_x[-1]]
                if pixel_value1 == 255:
                    pixel_x = pixel_x[::-1]
                    pixel_y = pixel_y[::-1]
            ...
            extracted_chains_3d = np.vstack(
                (extracted_chains_3d, pc_z))
            ...
            if self.multi_color_dlo:
                ...
                extracted_chains_3d = extracted_chains_3d[
                    extracted_chains_3d[:, 2]
                    > depth_threshold]

            tck, _u = interpolate.splprep(
                extracted_chains_3d.T, s=0.0005)
            # 1st fit, less points
            u_fine = np.linspace(0, 1, 300)
            x_fine, y_fine, z_fine = interpolate.splev(
                u_fine, tck)
            spline_pts = np.vstack(
                (x_fine, y_fine, z_fine)).T

            # 2nd fit, higher accuracy
            num_true_pts = int(np.sum(np.sqrt(np.sum(
                np.square(np.diff(spline_pts, axis=0)),
                axis=1))) * 1000)
            ...
            nodes = spline_pts[np.linspace(
                0, num_true_pts - 1,
                self.num_of_nodes).astype(int)]
            ...
            # add color
            pc_rgba = struct.unpack(
                'I', struct.pack('BBBB', 255, 40, 40, 255)
            )[0]
            points_with_color = [
                (float(p[0]), float(p[1]),
                 float(p[2]), int(pc_rgba))
                for p in init_nodes
            ]
