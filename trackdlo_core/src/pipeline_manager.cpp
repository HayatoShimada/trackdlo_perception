...
PipelineManager::PipelineManager(
  bool use_external_mask, bool multi_color_dlo,
  const std::vector<int> & lower, const std::vector<int> & upper)
{
  preprocessor_ = std::make_unique<trackdlo_core::ImagePreprocessor>(
    use_external_mask,
    multi_color_dlo, lower, upper);
...
void PipelineManager::initialize_tracker(
  const Eigen::MatrixXd & init_nodes,
  const std::vector<double> & converted_node_coord)
{
  Y_ = init_nodes;
...
PipelineResult PipelineManager::process(
  const cv::Mat & cur_image_orig, const cv::Mat & depth_image,
  const Eigen::MatrixXd & proj_matrix)
{
  ...
  pcl::PointCloud<pcl::PointXYZRGB>::Ptr downsampled_ptr =
    trackdlo_core::cuda::generate_downsampled_pointcloud(
  ...
  Eigen::MatrixXd X =
    result.cur_pc_downsampled.getMatrixXfMap().topRows(3).transpose().cast<double>();

  // Visibility mapping
  trackdlo_core::VisibilityResult vis_res = visibility_checker_->check_visibility(
    Y_, X,
    proj_matrix, mask,
  ...
  if (visible_nodes.size() < 3) {
    ...
    result.tracking_img = visualizer_->draw_tracking_image(
      cur_image_orig, cur_image, Y_,
      proj_matrix,
      result.not_self_occluded_nodes);
    return result;
  }
  zero_visible_count_ = 0;
  ...
  for (size_t i = 0; i + 1 < visible_nodes.size(); i++) {
    ...
    if (fabs(
        converted_node_coord_[visible_nodes[i + 1]] -
        converted_node_coord_[visible_nodes[i]]) <= d_vis_)
    {
  ...
  // run tracking
  tracker_.tracking_step(
    X, visible_nodes, visible_nodes_extended, proj_matrix, mask.rows,
    mask.cols);
  ...
  result.tracking_img = visualizer_->draw_tracking_image(
    cur_image_orig, cur_image, Y_, proj_matrix,
    result.not_self_occluded_nodes);
  ...
  for (int i = 0; i < Y_.rows(); ++i) {
    if (std::find(
        result.not_self_occluded_nodes.begin(), result.not_self_occluded_nodes.end(),
        i) == result.not_self_occluded_nodes.end())
    {
