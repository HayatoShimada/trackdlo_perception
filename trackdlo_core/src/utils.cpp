...
void remove_row(MatrixXd & matrix, unsigned int rowToRemove)
{
  ...
  if (rowToRemove < numRows) {
    matrix.block(rowToRemove, 0, numRows - rowToRemove, numCols) = matrix.bottomRows(
      numRows - rowToRemove);
  }
  ...
  std::vector<float> occluded_line_color)
{
  ...
  for (int i = 0; i < Y.rows(); i++) {
    visualization_msgs::msg::Marker cur_node_result;
    ...
    if (visible_nodes.size() != 0 &&
      std::find(visible_nodes.begin(), visible_nodes.end(), i) == visible_nodes.end())
    {
    ...
    visualization_msgs::msg::Marker cur_line_result;
  ...
  std::vector<float> occluded_line_color)
{
  ...
  for (size_t i = 0; i < Y.size(); i++) {
    ...
    if (visible_nodes.size() != 0 &&
      std::find(
        visible_nodes.begin(), visible_nodes.end(),
        static_cast<int>(i)) == visible_nodes.end())
    {
      cur_node_result.color.r = occluded_node_color[0];
      cur_node_result.color.g = occluded_node_color[1];
      cur_node_result.color.b = occluded_node_color[2];
    ...
    Eigen::Quaternionf q;
    Eigen::Vector3f vec1(0.0, 0.0, 1.0);
    Eigen::Vector3f vec2(Y[i](0, dim - 3) - Y[i - 1](0, dim - 3),
      Y[i](0, dim - 2) - Y[i - 1](0, dim - 2), Y[i](0, dim - 1) - Y[i - 1](0, dim - 1));
    q.setFromTwoVectors(vec1, vec2);
    ...
    cur_line_result.scale.z =
      sqrt(
      pow(
        Y[i](0, dim - 3) - Y[i - 1](0, dim - 3),
        2) +
      pow(
        Y[i](0, dim - 2) - Y[i - 1](0, dim - 2),
        2) + pow(Y[i](0, dim - 1) - Y[i - 1](0, dim - 1), 2));
