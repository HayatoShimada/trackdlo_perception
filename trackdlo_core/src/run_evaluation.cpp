...
public:
    ...
    initialized_(false)
  {
    ...
    tracking_evaluator_ = evaluator(
      rgb_count, trial_, pct_occlusion_, alg_, bag_file_,
...
private:
    ...
    const sensor_msgs::msg::PointCloud2::ConstSharedPtr & result_msg)
  {
    ...
    RCLCPP_INFO_STREAM(
      this->get_logger(),
      time_from_start << "; " << tracking_evaluator_.exit_time());
    ...
    if (time_from_start > tracking_evaluator_.recording_start_time()) {
      if (bag_file_ != 3) {
        ...
        if (time_from_start >
          tracking_evaluator_.recording_start_time() +
          tracking_evaluator_.wait_before_occlusion())
        {
          if (bag_file_ == 0) {
            int num_of_occluded_nodes =
              static_cast<int>(Y_track.rows() * tracking_evaluator_.pct_occlusion() / 100.0);
          ...
          tracking_evaluator_.wait_before_occlusion())
        {
          ...
          if (bag_file_ == 4) {
            cv::putText(
              eval_img, "occlusion", cv::Point(top_left_x - 190, bottom_right_y - 5),
              cv::FONT_HERSHEY_DUPLEX, 1.2, cv::Scalar(0, 0, 240), 2);
          } else {
            cv::putText(
              eval_img, "occlusion", cv::Point(top_left_x, top_left_y - 10),
              cv::FONT_HERSHEY_DUPLEX, 1.2, cv::Scalar(0, 0, 240), 2);
          }
        } else {
          cur_error = tracking_evaluator_.compute_error(Y_track, Y_true);
    ...
    for (int i = 0; i < image_coords_Y.rows(); i++) {
      ...
      if (row <= bottom_right_x && row >= top_left_x && col <= bottom_right_y &&
        col >= top_left_y)
      {
      ...
      if (i != image_coords_Y.rows() - 1) {
        cv::line(
          eval_img, cv::Point(row, col),
          cv::Point(
            static_cast<int>(image_coords_Y(i + 1, 0) / image_coords_Y(i + 1, 2)),
    ...
    if (save_images_) {
        ...
        fabs(diff - (tracking_evaluator_.image_counter() * time_step)) <= 0.1)
      {
        std::string dir;
        if (bag_file_ == 0) {
          dir = save_location_ + "images/" + alg_ + "_" + std::to_string(trial_) + "_" +
            std::to_string(pct_occlusion_) + "_stationary_frame_" + std::to_string(
            tracking_evaluator_.image_counter()) + ".png";
        } else if (bag_file_ == 1) {
          dir = save_location_ + "images/" + alg_ + "_" + std::to_string(trial_) + "_" +
            std::to_string(pct_occlusion_) + "_perpendicular_motion_frame_" + std::to_string(
            tracking_evaluator_.image_counter()) + ".png";
        } else if (bag_file_ == 2) {
          dir = save_location_ + "images/" + alg_ + "_" + std::to_string(trial_) + "_" +
            std::to_string(pct_occlusion_) + "_parallel_motion_frame_" + std::to_string(
            tracking_evaluator_.image_counter()) + ".png";
        } else if (bag_file_ == 3) {
          dir = save_location_ + "images/" + alg_ + "_" + std::to_string(trial_) + "_" +
            std::to_string(pct_occlusion_) + "_self_occlusion_frame_" + std::to_string(
            tracking_evaluator_.image_counter()) + ".png";
        } else if (bag_file_ == 4) {
          dir = save_location_ + "images/" + alg_ + "_" + std::to_string(trial_) + "_" +
            std::to_string(pct_occlusion_) + "_short_rope_folding_frame_" + std::to_string(
            tracking_evaluator_.image_counter()) + ".png";
        } else if (bag_file_ == 5) {
          dir = save_location_ + "images/" + alg_ + "_" + std::to_string(trial_) + "_" +
            std::to_string(pct_occlusion_) + "_short_rope_stationary_frame_" + std::to_string(
            tracking_evaluator_.image_counter()) + ".png";
        }
